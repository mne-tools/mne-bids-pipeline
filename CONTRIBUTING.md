...
You can add the pre-commit hook locally after installing `prek` with `pip install --group dev` or similar then running `prek install`, or run them manually via `prek run --all-files`.
