...
def run_inverse(subject, session=None):
    ...
    fname_fwd = op.join(deriv_path, bids_basename.update(suffix='fwd.fif'))
    fname_cov = op.join(deriv_path, bids_basename.update(suffix='cov.fif'))
    fname_inv = op.join(deriv_path, bids_basename.update(suffix='inv.fif'))
    ...
    for condition, evoked in zip(config.conditions, evokeds):
        ...
        fname_stc = op.join(deriv_path,
                            '_'.join([str(bids_basename.update(suffix=None)),
