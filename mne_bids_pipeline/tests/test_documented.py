...
def test_datasets_in_doc() -> None:
    ...
    # 1. Read cache and test jobs from the CircleCI workflow
    with open(root_path.parent / ".circleci" / "config.yml") as fid:
        circle_yaml = yaml.safe_load(fid.read())
    wf_jobs = circle_yaml["workflows"]["test_and_docs"]["jobs"]
    wf_jobs = [next(iter(job.items())) for job in wf_jobs if isinstance(job, dict)]
    cache_keys = dict()  # cache job name -> cache key
    for kind, job in wf_jobs:
        if kind != "cache_dataset":
            continue
        name = job["dataset"]
        assert job["name"] == f"cache_{name}"
        assert re.fullmatch(f"data-cache-{name}-[0-9]+", job["cache_key"])
        cache_keys[job["name"]] = job["cache_key"]
    caches = set(name[6:] for name in cache_keys)
    assert len(caches) == len(cache_keys)
    tests_list = list()
    for kind, job in wf_jobs:
        if kind != "test_dataset":
            continue
        name = job["test"]
        assert job["name"] == f"test_{name}"
        (cache_job,) = job["requires"]
        assert job["cache_key"] == cache_keys[cache_job], name
        assert name.startswith(cache_job[6:]), name
        tests_list.append(name)
    tests = set(tests_list)
    assert len(tests_list) == len(tests)
    # All tests that make reports must finish before building the docs
    (build_docs,) = [job for kind, job in wf_jobs if kind == "build_docs"]
    doc_tests = set(
        job["name"]
        for kind, job in wf_jobs
        if kind == "test_dataset" and job.get("copy_files", True)
    )
    assert set(build_docs["requires"]) == doc_tests
