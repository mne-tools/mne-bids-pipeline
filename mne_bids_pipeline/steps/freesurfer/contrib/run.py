...
def run(command, env={}, ignore_errors=False):
    ...
    process = Popen(
        command, stdout=PIPE, stderr=subprocess.STDOUT, shell=True, env=merged_env
    )
    while True:
...
parser = argparse.ArgumentParser(
    description="FreeSurfer recon-all + custom template generation."
)
parser.add_argument(
    "bids_dir",
...
parser.add_argument(
    "--n_cpus", help="Number of CPUs/cores available to use.", default=1, type=int
)
parser.add_argument(
    "--stages",
    help="Autorecon stages to run.",
    choices=[
        "autorecon1",
        "autorecon2",
        "autorecon2-cp",
...
parser.add_argument(
    "--steps",
    help="Longitudinal pipeline steps to run.",
...
parser.add_argument(
    "--template_name",
    help="Name for the custom group level template generated for this dataset",
    default="average",
)
parser.add_argument(
    "--license_file",
...
parser.add_argument(
    "--refine_pial_reconstruction_label",
...
parser.add_argument(
    "--multiple_sessions",
    help="For datasets with multiday sessions where you do not want to "
...
parser.add_argument(
    "--refine_pial",
    help="If the dataset contains 3D T2 or T2 FLAIR weighted images (~1x1x1), "
...
parser.add_argument(
    "--parcellations",
    help="Group2 option: cortical parcellation(s) to extract stats from.",
...
parser.add_argument(
    "--measurements",
    help="Group2 option: cortical measurements to extract stats for.",
    choices=[
        "area",
        "volume",
        "thickness",
...
parser.add_argument(
    "--skip_bids_validator", help="skips bids validation", action="store_true"
)
parser.add_argument(
    "--3T",
...
if args.bids_validator_config:
    run(
        "bids-validator --config {config} {bids_dir}".format(
            config=args.bids_validator_config, bids_dir=args.bids_dir
        )
    )
...
if args.refine_pial_acquisition_label and not args.refine_pial_reconstruction_label:
    ...
    ar_t2 = "*acq-%s*_rec-%s*" % (
        args.refine_pial_acquisition_label,
        args.refine_pial_reconstruction_label,
    )
else:
    ar_t2 = "*"
...
if glob(os.path.join(args.bids_dir, "sub-*", "ses-*")):
    ...
    for subject_label in subjects:
        session_dirs = glob(
            os.path.join(args.bids_dir, "sub-%s" % subject_label, "ses-*")
        )
...
if args.analysis_level == "participant":
    ...
    for fst in fst_links_to_make:
        try:
            os.symlink(
                os.path.join(os.environ["SUBJECTS_DIR"], fst),
            ...
            if e.errno == errno.EEXIST:
                ...
                print(
                    "ERROR: Symbolic link to {0} unable to be created because: {1}".format(
                        fst, str(e)
                    )
                )
                raise e
    ...
    for subject_label in subjects_to_analyze:
        if glob(os.path.join(args.bids_dir, "sub-%s" % subject_label, "ses-*")):
            ...
            sessions = set(
                [os.path.normpath(t1).split(os.sep)[-3].split("-")[-1] for t1 in T1s]
            )
            ...
            if len(sessions) > 0 and longitudinal_study is True:
                timepoints = [
                    "sub-%s_ses-%s" % (subject_label, session_label)
                    for session_label in sessions
                ]
                if "cross-sectional" in args.steps:
                    ...
                    for session_label in sessions:
                        ...
                        for T1 in T1s:
                            if (
                                round(max(nibabel.load(T1).header.get_zooms()), 1) < 1.0
                                and args.hires_mode == "auto"
                            ) or args.hires_mode == "enable":
                                input_args += " -hires"
                            input_args += " -i %s" % T1
                        ...
                        if args.refine_pial == "T2":
                            for T2 in T2s:
                                if (
                                    max(nibabel.load(T2).header.get_zooms()) < 1.2
                                ) | args.allow_lowresT2:
                            ...
                            for FLAIR in FLAIRs:
                                if (
                                    max(nibabel.load(FLAIR).header.get_zooms()) < 1.2
                                ) | args.allow_lowresT2:
                        ...
                        cmd = "recon-all -subjid %s -sd %s %s %s -openmp %d" % (
                            fsid,
                            output_dir,
                            input_args,
                            stages,
                        ...
                        resume_cmd = "recon-all -subjid %s -sd %s %s -openmp %d" % (
                            fsid,
                            output_dir,
                            stages,
                            args.n_cpus,
                        ...
                        if os.path.isfile(
                            os.path.join(output_dir, fsid, "scripts/IsRunning.lh+rh")
                        ):
                            ...
                            os.path.join(
                                output_dir, fsid, "label/BA_exvivo.thresh.ctab"
                            )
                        ):
                            print("SUBJECT ALREADY SEGMENTED, SKIPPING")
                        elif os.path.exists(os.path.join(output_dir, fsid)):
                            print(
                                "SUBJECT DIR ALREADY EXISTS (without IsRunning.lh+rh), RUNNING COMMAND:"
                            )
                            print(resume_cmd)
                ...
                if "template" in args.steps:
                    ...
                    cmd = "recon-all -base %s -sd %s %s %s -openmp %d" % (
                        fsid,
                        output_dir,
                        input_args,
                        stages,
                    ...
                    if os.path.isfile(
                        ...
                        os.path.join(output_dir, fsid, "label/BA_exvivo.thresh.ctab")
                    ):
                ...
                if "longitudinal" in args.steps:
                    for tp in timepoints:
                        ...
                        cmd = "recon-all -long %s %s -sd %s %s -openmp %d" % (
                            tp,
                            fsid,
                            output_dir,
                            stages,
            ...
            if not T1s:
                print(
                    "No T1w nii files found for subject %s. Skipping subject."
                    % subject_label
                )
                continue
    ...
    if len(subjects_to_analyze) > 1:
        ...
        cmd = (
            "make_average_subject --no-symlink --no-vol --out "
            + args.template_name
            + " --subjects "
            + " ".join(fsids)
        )
        ...
        for subject_label in subjects_to_analyze:
            for hemi in ["lh", "rh"]:
                tif_file = os.path.join(
                    output_dir, args.template_name, hemi + ".reg.template.tif"
                )
                fsid = "sub-%s" % subject_label
                sphere_file = os.path.join(output_dir, fsid, "surf", hemi + ".sphere")
                reg_file = os.path.join(
                    output_dir, fsid, "surf", hemi + ".sphere.reg." + args.template_name
                )
                cmd = "mris_register -curv %s %s %s" % (sphere_file, tif_file, reg_file)
                run(
                    cmd,
                    env={"SUBJECTS_DIR": output_dir, "FS_LICENSE": args.license_file},
                )
    else:
    ...
    if glob(os.path.join(output_dir, "sub-*_ses-*.long.sub-*")):
        for s in subjects_to_analyze:
            fs_sessions = sorted(
                glob(
                    os.path.join(output_dir, "sub-{s}_ses-*.long.sub-{s}*".format(s=s))
                )
            )
            if fs_sessions:
                ...
                raise Exception(
                    "No freesurfer sessions found for %s in %s" % (s, output_dir)
                )
    else:
        for s in subjects_to_analyze:
            if os.path.isdir(os.path.join(output_dir, "sub-" + s)):
                ...
                raise Exception(
                    "No freesurfer subject found for %s in %s" % (s, output_dir)
                )
    ...
    if len(subjects) > 0:
            ...
            for h in ["lh", "rh"]:
                for m in args.measurements:
                    table_file = os.path.join(
                        table_dir, "{h}.{p}.{m}.tsv".format(h=h, p=p, m=m)
                    )
                    if os.path.isfile(table_file):
                        warn("Replace old file %s" % table_file)
                        os.remove(table_file)
                    cmd = (
                        "python2 `which aparcstats2table` --hemi {h} --subjects {subjects} --parc {p} --meas {m} "
                        "--tablefile {table_file}".format(
                            h=h, subjects=subjects_str, p=p, m=m, table_file=table_file
                        )
                    )
                    print(
                        "Creating cortical stats table for {h} {p} {m}".format(
                            h=h, p=p, m=m
                        )
                    )
                    run(
        ...
        if os.path.isfile(table_file):
            ...
            os.remove(table_file)
        cmd = (
            "python2 `which asegstats2table` --subjects {subjects} --meas volume --tablefile {"
    ...
    def extract_euler(logfile):
        ...
        if len(results) != 1:
            raise Exception(
                "Euler number could not be extracted from {}".format(logfile)
            )
    ...
    if len(subjects) > 0:
        ...
        for subject in subjects:
            ...
            df_subject = pd.DataFrame(
                {"subject": [subject], "lh_euler": [lh_euler], "rh_euler": [rh_euler]},
